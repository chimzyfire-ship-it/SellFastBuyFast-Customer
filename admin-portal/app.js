...
function authPage(mode = "sign-in", message = "") {
  ...
  app.innerHTML = `<main id="main" class="auth"><section class="auth-story"><a class="brand" href="#/sign-in"><span class="brand-mark">${icon("store")}</span><span>SellFastBuyFast<small>OPERATIONS WORKSPACE</small></span></a><div class="auth-copy"><div class="eyebrow">Behind every great marketplace</div><h1>Good commerce.<br><em>Thoughtfully managed.</em></h1><p>One connected workspace for the people who keep our marketplace moving.</p><div class="auth-feature">${icon("store")} Merchant confidence, from the first review.</div><div class="auth-feature">${icon("shield")} Customer care, with the full picture.</div><div class="auth-feature">${icon("scale")} Every decision, clearly accounted for.</div></div><div class="auth-footer">SELLFASTBUYFAST · MARKETPLACE OPERATIONS</div></section><section class="auth-form-side"><div class="auth-card"><div class="eyebrow">Authorized staff only</div><h2>${recover ? "Recover your access" : reset ? "Choose a new password" : "Welcome to operations"}</h2><p>${recover ? "Enter your work email to request a password reset." : reset ? "Set a strong, unique password for your account." : "Sign in with your staff account to continue."}</p>${message ? notice(message) : ""}<form data-form="${mode}">${!reset ? field("email", "Work email", "", "email", 'required autocomplete="username" maxlength="254"') : ""}${!recover ? field("password", reset ? "New password" : "Password", "", "password", `required autocomplete="${reset ? "new-password" : "current-password"}" ${reset ? 'minlength="12"' : ""}`) : ""}${reset ? field("confirmPassword", "Confirm new password", "", "password", 'required autocomplete="new-password" minlength="12"') : ""}${errorSlot}<button class="btn primary" type="submit">${recover ? "Send recovery email" : reset ? "Update password" : "Sign in"} ${icon("arrow")}</button></form><div class="auth-note">${recover || reset ? '<a href="#/sign-in">Back to sign in</a>' : '<a href="#/recover">Forgot your password?</a>'}<p>Access is granted by your workspace administrator. Customer and merchant accounts do not grant staff access.</p></div></div></section></main>`;
}
