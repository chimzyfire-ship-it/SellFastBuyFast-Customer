  ...
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      await tx.insert(auditEvents).values({
        actorId: req.user!.id,
        action: 'merchant.onboarding_started',
...
/**
 * Retired to ensure Operations cannot approve a verification without moving
 * the registration lifecycle at the same time. Use /registration/decision.
 */
vendorRouter.post('/merchant/:merchantId/verification/review', (_req: Request, _res: Response) => {
  throw errors.conflict('VERIFICATION_ENDPOINT_RETIRED', 'Use POST /registration/decision to review merchant registration.');
});
