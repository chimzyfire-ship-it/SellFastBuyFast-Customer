import { freshStaff } from "../admin/admin.store.js";
import { requireFinancialEnabled } from "../admin/admin.legacy.js";
...
const DeliverySchema = z.object({
  deliveryEvidenceUrl: z
    .string()
    .url()
    .refine((value) => value.startsWith("https://"), "Evidence must use HTTPS"),
...
async function assertMerchantAccess(
  ...
  if (req.user!.merchantIds.includes(merchantId)) return;
  await freshStaff(req.user!, ["operations_admin", "support_agent"]);
}
...
fulfilmentRouter.get(
  ...
  async (req: Request, res: Response) => {
    try {
      await assertMerchantAccess(req, req.params.merchantId);
      const list = await db
        .select()
        .from(orders)
      ...
      res.json({
        ...
        data: list.map((order) => ({
          ...
          shipment:
            shipmentRows.find((shipment) => shipment.orderId === order.id) ??
            null,
        })),
      });
    } catch (err) {
  ...
  async (req: Request, res: Response) => {
    try {
      const result = await db.transaction(async (tx) => {
        ...
        await assertMerchantAccess(req, order.merchantId);
        ...
        await tx
          .insert(outboxEvents)
          .values({ type: "order.accepted", payload: { orderId: order.id } });
      ...
      const result = await db.transaction(async (tx) => {
        ...
        if (order.status !== "processing")
          throw errors.invalidTransition(
            "Only processing orders can be packed.",
          );
        ...
        if (shipment.status !== "pending")
          throw errors.conflict(
            "SHIPMENT_ALREADY_PACKED",
            "Shipment is not awaiting packing.",
          );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await db.transaction(async (tx) => {
        ...
        await tx
          .update(orders)
          .set({ trackingCode: parsed.data.trackingCode })
          .where(eq(orders.id, order.id));
        await tx
          .insert(outboxEvents)
          .values({
            type: "order.shipped",
            payload: { orderId: order.id, shipmentId: shipment.id },
          });
        await tx.insert(notifications).values({
          userId: order.buyerId,
          type: "order_shipped",
  ...
  async (req: Request, res: Response) => {
      ...
      res
        ...
        .json({
          ...
          error: {
            ...
            message: "Carrier webhook is not configured.",
          },
        });
      return;
    }
      ...
      res
        ...
        .json({
          ...
          error: {
            ...
            message: "Invalid carrier webhook signature.",
          },
        });
      return;
    }
    ...
    try {
      ...
      const result = await db.transaction(async (tx) => {
        ...
        if (!providerEvent)
          throw errors.internal("Unable to persist carrier webhook event.");
        ...
        if (!event.deliveryEvidenceUrl) {
          throw errors.validation(
            "A delivered carrier event requires deliveryEvidenceUrl.",
          );
        }
        if (
          event.occurredAt &&
          event.occurredAt.getTime() > Date.now() + 5 * 60_000
        ) {
          throw errors.validation(
            "Carrier delivery time cannot be in the future.",
          );
        }
